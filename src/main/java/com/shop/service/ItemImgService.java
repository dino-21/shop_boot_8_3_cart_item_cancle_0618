...
public class ItemImgService {
    ...
    // 상품이미지 저장하는 메서드
    // 1 저장할 상품 이미지 정보를 담고 있는 itemImg 엔티티 객체
    // 2 클라이언트로부터 업로드된 이미지 파일을 나타내는 MuㅣtipartFile 객체
    // 파일업로드 또는 DB저장 중 예외가 발생할 수 있음
    public void saveItemImg(ItemImg itemImg, MultipartFile itemImgFile) throws Exception {
        ...
        // 상품 이미지 정보 업데이트 및 저장
        itemImg.updateItemImg(oriImgName, imgName, imgUrl);  // 원본 파일이름, 저장된 파일이름, URL로 업데이트
        itemImgRepository.save(itemImg);  //DB저장
    }
